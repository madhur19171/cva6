...
void __attribute__ ((noinline)) generateSendPacket(uint64_t *sendPacket, uint64_t *receivedPacket, uint32_t dest_x, uint32_t dest_y, uint32_t messageNumber){
	...
	for(int i = 0; i < packetLength; i++){
		...
		if(i == 0){
			...
			flit |= 2lU << 62;
			flit |= receivedPacket[i] * (SOURCE_Y * 2 + SOURCE_X);
		}
...
int main() {
	...
	uint64_t *b = (uint64_t *) (0x80100000U + TRANSFER_LENGTH);   // Receiver
	...
	writeToDebug((SOURCE_Y * 2 + SOURCE_X));
